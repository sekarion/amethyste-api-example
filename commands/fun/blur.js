...
class Beautiful extends Command {
    ...
    async run(msg) {
        let msgSplitinfo = msg.content.split(' ').splice(1);
        let args = minimist(msgSplitinfo, {boolean: ['b'], string: ['i']});
        //parse the args blur
        ...
        if(blur <= 0){
            return msg.channel.send("The option blur need positive (min 1 and max 30)");
        }
        if(blur >30){
            return msg.channel.send("The option blur max is 30");
        }
        let msgSplit = args._.join(" ");
        let usr;
        ...
        //generate the image
        let imgBuffer = await this.bot.ameAPI.generate("blur", {
            url: usr.user.displayAvatarURL({
                format: "png",
                size: 1024
            }), blur: blur
        }).catch(err=> this.bot.log.error(err));
        return await msg.channel.send(new MessageAttachment(imgBuffer, `blur-${new Date("now")}.png`));
    }
}
